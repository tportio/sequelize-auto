...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables).sort(), function(table, _callback){
      ...
      var tableName = self.options.interfacePascalCase ? tsHelper.def.PascalCase(table) : table;
      ...
      if(!self.options.typescript){
        ...
        text[table] = tsHelper.model.getModelFileStart(self.options.indentation, spaces, table, tableName);
      }
      ...
      _.each(fields, function(field, i){
        ...
        if (self.tables[table][field].type === "USER-DEFINED" && !! self.tables[table][field].special) {
          self.tables[table][field].type = "ENUM(" + self.tables[table][field].special.sort().map(function(f){ return quoteWrapper + f + quoteWrapper; }).join(', ') + ")";
        }
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            ...
            tsVal = 'DataTypes.' + self.tables[table][field][attr];
          } else {
        ...
        if(self.options.typescript) tsTableDef += tsHelper.def.getMemberDefinition(spaces, fieldName, tsVal, defaultVal, tsAllowNull);
      });
      ...
      if(self.options.typescript) typescriptFiles[0] += tsHelper.def.getTableDefinition(tsTableDef, table, tableName, spaces);
      ...
      if(self.options.typescript) typescriptFiles[0] += tsHelper.def.getDefinitionFileEnd();
      if(self.options.typescript) typescriptFiles[1] = tsHelper.model.generateTableModels(_.keys(text), self.options.spaces, self.options.indentation, self.options.interfacePascalCase, self.options.camelCase, self.options.camelCaseForFileName);
